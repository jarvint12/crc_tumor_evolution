...
project_name = 'mlp_basemodel_{}'.format(data_mode)
...
last_run_log_file_final_runs=os.path.join("logs", data_mode, "train_{}_all_runs.log".format(model_version))
...
def cv_train_network(net,criterion, valid_criterion,epochs,optimizer,trainloader, validloader,
    ...
    for i in range(epochs):
        ...
        for sequences, _, mut_landscape, labels in trainloader:
            ...
            optimizer.zero_grad()
            out=net(sequences, mut_landscape)
            out=out.squeeze()
        ...
        accuracy, tot_valid_loss = compute_accuracy(device, net, validloader, valid_criterion, 
                                                "VALID", "mlp", verbose = False, cv=True)
        if wandb!=None:
...
def train_network(net,criterion, valid_criterion,epochs,optimizer,trainloader, validloader,
    ...
    for i in range(epochs):
        ...
        if is_scheduler:
            scheduler.step()
        tot_loss/=tot_items
        accuracy, tot_valid_loss = compute_accuracy(device, net, validloader, valid_criterion, "VALID", "mlp", verbose = False, cv = True)
...
if cv:
    ...
    for i in range(150): #Test with 30 different hyperparameter combinations
        ...
        for (train_ids, test_ids) in kfold.split(combined_valid_train):
            ...
            run = wandb.init(project="mlp_classes")
        ...
        if avg_valid_acc>greatest_avg_valid_acc:
            ...
            write_to_log_file(learning_rate, optimizer_type, weight_decay, momentum, is_scheduler, T_max,
                             smoothing, train_batch_size, valid_batch_size, cv_log_file, model_version, 
                              ...
                              model_script, run_name, dt_string, norm=norm, acc_list=valid_accuracies)
...
if train:
    ...
    for i in range(1000):
        ...
        greatest_acc = train_network(net,criterion, valid_criterion,epochs,optimizer,trainloader, validloader,
                          is_scheduler,scheduler, greatest_acc_overall, model_path, wandb=None, early_stop=None)
        ...
        if greatest_acc>greatest_acc_overall:
            ...
            write_to_log_file(learning_rate, optimizer_type, weight_decay, momentum, is_scheduler, T_max,
                             smoothing, train_batch_size, valid_batch_size, log_file, model_version, 
                              ...
                              model_script, run_name, dt_string, norm)
...
valid_criterion=nn.CrossEntropyLoss(weight=valid_class_weights)
accuracy, tot_valid_loss = compute_accuracy(device, model, testloader, valid_criterion, "TEST", "mlp", verbose = True, cv = False)

print("Accuracy for the test data:",accuracy)


accuracy, tot_valid_loss = compute_accuracy(device, model, validloader, valid_criterion, "VALID", "mlp", verbose = True, cv = False)

print("Accuracy for the valid data:",accuracy)
