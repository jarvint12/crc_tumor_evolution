...
from model_skeletons.cnn_basemodel.base_cnn_v5 import base_cnn
from compute_accuracy import compute_accuracy
from class_weights_cv import get_class_weights

random.seed(60)#(42) #For validation dataset creation

data_mode = "clu4" #"clu3" "clu4" "clu431" "clu1"
model_version="base_cnn_v5"
model_script=os.path.join("model_skeletons", "cnn_basemodel", "{}.py".format(model_version)) #Used to read the file for logs
project_name = 'base_cnn_{}'.format(data_mode)
...
input_to_matrix=os.path.join("process_input", "input_to_1D_seq_matrix.py") #Used to read the file for logs
...
test_file="data/clu9/sompred_crc9_clu9_pyri_mut_predict.matrix"
combined_valid_train_file="data/{0}/sompred_crc9_{0}_pyri_mut_train.matrix".format(data_mode)
    
    
combined_valid_train_input, combined_valid_train_target = parse_matrices(combined_valid_train_file, classes)
test_input, test_target = parse_matrices(test_file, classes)



def get_valid_dataset(trainset_input, trainset_target):
    classes=dict()
    for index, target in enumerate(trainset_target):
        current_class=target.item()
        if not current_class in classes:
            classes[current_class]=list()
        classes[current_class].append(index)
    
    train_tensors = []  # List to store selected tensors for each class
    train_targets = []  # List to store not selected tensors for each class
    valid_tensors = []  # List to store selected tensors for each class
    valid_targets = []  # List to store not selected tensors for each class
    
    for target in classes:
        num_indices_to_select = int(0.85 * len(classes[target]))
        train_indices = random.sample(classes[target], num_indices_to_select)
        valid_indices = [index for index in classes[target] if not index in train_indices]
        
        train_tensor = trainset_input[train_indices]
        train_tensors.append(train_tensor)
        train_targets.append(trainset_target[train_indices])

        valid_tensor = trainset_input[valid_indices]
        valid_tensors.append(valid_tensor)
        valid_targets.append(trainset_target[valid_indices])

    trainset_input = torch.cat(train_tensors, dim=0)
    trainset_target = torch.cat(train_targets, dim=0)
    valid_input = torch.cat(valid_tensors, dim=0)
    valid_target = torch.cat(valid_targets, dim=0)
    return trainset_input, trainset_target, valid_input, valid_target

trainset_input, trainset_target, valid_input, valid_target = get_valid_dataset(combined_valid_train_input, combined_valid_train_target)
...
combined_valid_train=FeatureDataset(data=combined_valid_train_input, labels=combined_valid_train_target)

print(trainset_target.shape)
print(torch.sum(trainset_target!=1),torch.sum(trainset_target!=7))
print("Train neg, pos:",trainset_target.shape[0]-torch.sum(trainset_target==1)-torch.sum(trainset_target==7),
      torch.sum(trainset_target==1)+torch.sum(trainset_target==7))
...
if cv:
    ...
    for i in range(150): #Test with 30 different hyperparameter combinations
        ...
        for (train_ids, test_ids) in kfold.split(combined_valid_train):
            ...
            net = base_cnn(many_classes).to(device)
            ...
            if many_classes:
                ...
                raise RuntimeError("Implement class weights for binary case")
                #criterion = nn.BCELoss(weight=class_weights)

            run = wandb.init(project='CNN_1D')
        ...
        if avg_valid_acc>greatest_avg_valid_acc:
            ...
            write_to_log_file(learning_rate, optimizer_type, weight_decay, momentum, is_scheduler, T_max,
                             smoothing, train_batch_size, valid_batch_size, cv_log_file, model_version, 
                              input_to_matrix, 
                              avg_valid_acc, model_path, compute_accuracy_file,
...
accuracy, loss = \
compute_accuracy(device, model, validloader, None, "VALID", verbose=True, cv=False)
print("Accuracy:",accuracy)