...
class WeightedMSELoss(nn.Module):
    ...
    def forward(self, sequences, targets, outputs):
        ...
        # Calculate weights based on correct classes (replace this with your logic)
...
class Transformer(nn.Module):
    def __init__(self, d_model=84, nhead=8, num_encoder_layers=6, num_decoder_layers=6):
        ...
        self.positional_encoder_input = PositionalEncoding(
            dim_model=d_model, max_len=34
        )
        self.positional_decoder_input = PositionalEncoding(
            dim_model=d_model, max_len=3
        )
    ...
    def forward(self, src, tgt, tgt_mask=None):
        ...
        src = self.positional_encoder_input(src)
        tgt = self.positional_decoder_input(tgt)
    ...
    def get_tgt_mask(self, size, device) -> torch.tensor:
        ...
        #mask = torch.triu(torch.ones((size, size), device=device), diagonal=1).bool() #batch_size removed
