...
class cnn(nn.Module):
    def __init__(self):
        ...
        self.conv3 = nn.Conv1d(12, 15, 3, padding=1)
        self.conv1_2 = nn.Conv2d(21, 15, (4,33))
        self.norm3=nn.BatchNorm1d(510)
        self.fc1=nn.Linear(510,200)
    ...
    def forward(self, x):
        ...
        y=self.conv2(y)
        y=self.dropout(y)
        y=F.relu(y) # (batch_size, 12, 33)
        y=self.norm2(y)
        
        y=self.conv3(y)
        y=self.dropout(y)
        y=F.relu(y) # (batch_size, 15, 33)
        
        y2=self.conv1_2(x)
        y2=F.relu(y2).squeeze(dim=2) # (batch_size, 21, 4, 33) -> (batch_size, 15, 1)
        y=torch.cat((y,y2), dim=2) # (batch_size, 15, 34)
        
        y = y.view(-1, self.num_flat_features(y)) # (batch_size, 510)
        y=self.norm3(y)
