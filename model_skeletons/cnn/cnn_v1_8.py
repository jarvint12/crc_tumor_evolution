...
class cnn(nn.Module):
    ...
    def forward(self, x):
        ...
        y=self.conv1(x)
        y=self.dropout(y)
        y=F.relu(y) # (batch_size, 6, 1, 31)
        y=y.squeeze(dim=2) # (batch_size, 6, 31)
        
        y=self.conv2(y)
        y=self.dropout(y)
        y=F.relu(y) # (batch_size, 12, 29)
        
        y=self.conv3(y)
        y=self.dropout(y)
        y=F.relu(y) # (batch_size, 15, 27)
        
        y2=self.conv1_2(x)
        y2=F.relu(y2).squeeze(dim=2) # (batch_size, 21, 4, 33) -> (batch_size, 15, 1)
        y=torch.cat((y,y2), dim=2) # (batch_size, 15, 28)
